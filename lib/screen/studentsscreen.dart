...
Future<void> getStudentsByClass(Classe selectedClass) async {
  Response response = await http.get(Uri.parse("http://10.0.2.2:8081/classes/${selectedClass.codClass}/etudiants"));
  ...
  if (response.statusCode == 200) {
    ...
    if (data['_embedded'] != null && data['_embedded']['etudiants'] is List) {
      ...
      List<Student> studentsInClass = etudiantsData.map((json) => Student.fromJson(json)).toList();
      setState(() {
        students = studentsInClass;
        print(students);
      });
    } else {
      throw Exception("No 'etudiants' data found or data structure is invalid");
    }
  } else {
